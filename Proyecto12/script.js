...
form.addEventListener("submit",(e)=>{
    e.preventDefault()
    let valor = parseInt(parrafos.value)
    let random = Math.floor(Math.random() * text.length)

   if(isNaN(valor) || valor < 0 || valor > 9){
     lorem.innerHTML = `<p>${text[random]}</p>`  
   }else {
    let temporal = text.slice(0, valor);
    temporal = temporal.map(function (item) {
        return `<p>${item}</p>`;
    }).join("\n")
      
    lorem.innerHTML = temporal;
  }
